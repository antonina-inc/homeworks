...
if (confirm('Почати тестування?')) {
    ...
    const totalSum = dataList.reduce((prevSum, obj) => prevSum + obj.price, 0);
    console.log(`Загальна вартість - ${totalSum}`);
    // 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).
    const numberPriceReduction = dataList.reduce((prevSum, obj) => (obj.price < obj.old_price ? prevSum + 1 : prevSum), 0);
    console.log(`кількість товарів, у яких ціна зменшилась - ${numberPriceReduction}`);
    // 3) Товари, які доступні (sell_status:"available")
    const availableGoods = dataList.filter((goods) => goods.sell_status === 'available');
    ...
    const newListAvailableGoods = availableGoods.map((goods) => ({
        id: goods.id,
        price: goods.price,
        old_price: goods.old_price,
        usd_price: goods.usd_price
    }));
    console.log(newListAvailableGoods);
}
