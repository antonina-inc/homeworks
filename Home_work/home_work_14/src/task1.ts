if (confirm('Почати тестування?')) {
	...
	const numberSite = sitesInfo.reduce((prevSum, site) =>
	...
	const numberSiteMore100k = sitesInfo.reduce((prevNum, site) => {
		const totalSumInvest = site.sponsors.reduce((prevSum, sponsor) =>
	...
	function getSponsorsList(arr: Site[]) {
	...
	function getBiggestProfitYear(arr: Site[]) {
		let biggestProfit = 0
